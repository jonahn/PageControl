...
#import "JNPageControl.h"
@interface JNPageControl ()
    ...
    if (_numberOfPages != numberOfPages) {
        ...
        for (NSInteger i = 0; i < numberOfPages; i++) {
            if (i == _currentPage) {
                ...
                view.backgroundColor = self.currentPageIndicatorColor;
                ...
                view.backgroundColor = self.pageIndicatorColor;
                view.layer.cornerRadius = _indicatorHeight/2;
                view.layer.masksToBounds = YES;
                viewX += _indicatorWidth + _indicatorMargin;
                [self.contentView addSubview:view];
...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary *)change context:(void *)context{
    ...
    if (rate >= 0.0f && rate <= _numberOfPages - 1) {
        ...
        CGFloat r,g,b,a;
        [self.currentPageIndicatorColor getRed:&r green:&g blue:&b alpha:&a];
        CGFloat or,og,ob,oa;
        [self.pageIndicatorColor getRed:&or green:&og blue:&ob alpha:&oa];
        
        CGFloat oldAlpha = (oldView.frame.size.width - self.indicatorWidth)/ (self.currentIndicatorWidth - self.indicatorWidth);
        UIColor *oldColor = [UIColor colorWithRed:r green:g blue:b alpha:oldAlpha];
        if (oldAlpha <= 0.5) {
            oldColor = [UIColor colorWithRed:or green:og blue:ob alpha:1-oldAlpha];
        }
        CGFloat newAlpha = (currentView.frame.size.width - self.indicatorWidth)/ (self.currentIndicatorWidth - self.indicatorWidth);
        UIColor *newColor = [UIColor colorWithRed:r green:g blue:b alpha:newAlpha];
        if (newAlpha <= 0.5 && offSetX < 0) {
            newColor = [UIColor colorWithRed:or green:og blue:ob alpha:1-newAlpha];
        }
        if (newAlpha <= 0.5 && offSetX >= 0) {
            newAlpha = 0.5;
          newColor = [UIColor colorWithRed:r green:g blue:b alpha:newAlpha];
        }
        currentView.backgroundColor = newColor;
        oldView.backgroundColor = oldColor;
        if (offSetX < 0) {
            if (currentIndex + 1 < self.indicatorViews.count) {
                UIView *view = self.indicatorViews[currentIndex + 1];
                view.backgroundColor = self.pageIndicatorColor;
            }
        }
    }
